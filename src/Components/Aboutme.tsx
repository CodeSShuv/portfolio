

const Aboutme = () => {
    return (
        <>
            <section className='about-me flex flex-col items-start'>
                <h2 className='text-3xl p-4 text-black vt323-regular'>Who am I? </h2>
                <div className='p-5'>

                    <ul className='text-black text-1xl vt323-regular
                        charList'>
                        <li >I'm Shuvam Gautam, a self-taught programmer and a BCA student in my 3rd semester.</li>

                        <li>
                            I specialize in full-stack web development using React, Node.js, MongoDB, and TypeScript.
                        </li>

                        <li >I enjoy solving problems and building intuitive, user-friendly websites.
                        </li>
                        <li >I'm passionate about continuously learning new technologies and improving my skills.
                        </li>
                    </ul>

                </div>

            </section>
        </>
    )
}

export default Aboutme